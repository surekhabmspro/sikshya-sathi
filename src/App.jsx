...
const WARN = "#9A5B12";
const WARN_BG = "#FCF0DA";

// Elevation scale — used for the "premium, elevated" card/button look.
const SHADOW = {
  sm: "0 1px 2px rgba(33,30,26,0.06), 0 1px 1px rgba(33,30,26,0.04)",
  md: "0 4px 14px rgba(33,30,26,0.08), 0 1px 3px rgba(33,30,26,0.06)",
  lg: "0 12px 28px rgba(33,30,26,0.14), 0 4px 10px rgba(33,30,26,0.08)",
...
function Button({ children, onClick, variant="primary", size="md", disabled, style, icon:Icon, type }) {
  const variants = {
    ...
    secondary: { background:"#fff", color:ACCENT, border:`1.5px solid ${BORDER}`, boxShadow:SHADOW.sm },
    ghost:     { background:ACCENT_LIGHT, color:ACCENT, border:"none", boxShadow:"none" },
    danger:    { background:"#fff", color:DANGER, border:`1.5px solid ${DANGER_BG}`, boxShadow:SHADOW.sm },
  };
  const sizes = {
    sm: { padding:"8px 14px", fontSize:14.5 },
    md: { padding:"12px 20px", fontSize:15.5 },
    lg: { padding:"15px 26px", fontSize:17 },
  };
  return (
    <button
      type={type||"button"}
...
function Card({ children, onClick, style }) {
  return (
    <div onClick={onClick}
      ...
      style={{ background:SURFACE, border:`1px solid ${BORDER}`, borderRadius:18, padding:20, cursor:onClick?"pointer":"default", boxShadow:SHADOW.sm, ...style }}>
      {children}
    </div>
  );
}
function SectionLabel({ children }) {
  return <div style={{ fontSize:14.5, letterSpacing:"0.06em", textTransform:"uppercase", color:INK_SOFT, marginBottom:11, fontWeight:700 }}>{children}</div>;
}
function StatusPill({ status }) {
  ...
  return <span style={{ background:s.bg, color:s.color, fontSize:14, fontWeight:700, padding:"4px 12px", borderRadius:999 }}>{s.label}</span>;
}
...
function ErrorMsg({ msg }) {
  return <div style={{ display:"flex", alignItems:"center", gap:9, background:DANGER_BG, borderRadius:12, padding:"12px 15px", fontSize:14.5, color:DANGER, margin:"10px 0", fontWeight:500 }}><AlertCircle size={17}/>{msg}</div>;
}
...
function MaterialsHint({ count, chapterTitle }) {
  ...
  return (
    <div style={{ fontSize:13.5, color: count>0?ACCENT:"#9A5B12", background: count>0?"#E4EFE6":"#FBEBD3", borderRadius:8, padding:"6px 10px", marginBottom:8, display:"flex", alignItems:"center", gap:6 }}>
      <FileText size={13}/>
  ...
  return(
    <div>
      <select
        ...
        style={{width:"100%",border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",background:"#fff",color:value?INK:"#8A8275"}}>
      ...
      {showAdd&&(
        <div style={{display:"flex",gap:8,marginTop:8}}>
          <input autoFocus value={newTitle} onChange={(e)=>setNewTitle(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&submitNew()} placeholder="नयाँ अध्यायको नाम लेख्नुहोस्" style={{flex:1,border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
          <button onClick={submitNew} disabled={adding||!newTitle.trim()} style={{background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"10px 16px",fontWeight:700,fontSize:14.5,cursor:"pointer"}}>{adding?"...":"थप्नुहोस्"}</button>
        </div>
      )}
    </div>
  );
...
function GetStartedCard({ chapters, materialsCount, lessons, onGoMaterials, onGoPlanner }) {
  ...
  const Step=({ done, num, title, sub, onClick })=>(
    <div onClick={onClick} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 6px",cursor:onClick?"pointer":"default",opacity:done?0.55:1}}>
      <div style={{width:28,height:28,borderRadius:"50%",background:done?ACCENT:"#F4EFE3",color:done?"#fff":"#7A6F3E",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:14.5,flexShrink:0}}>{done?"✓":num}</div>
      <div style={{flex:1}}>
        <div style={{fontSize:15,fontWeight:700,color:INK,textDecoration:done?"line-through":"none"}}>{title}</div>
        {sub&&<div style={{fontSize:13.5,color:"#8A8275",marginTop:1}}>{sub}</div>}
      </div>
  ...
  return(
    <Card style={{marginBottom:20}}>
      <div style={{fontSize:16,fontWeight:700,color:INK,marginBottom:4}}>👋 सुरु गर्नुहोस्</div>
      <div style={{fontSize:14,color:"#8A8275",marginBottom:8}}>तीन सजिलो चरणमा शिक्षा साथी प्रयोग गर्नुहोस्</div>
    ...
    <div style={{minHeight:"100vh",background:`radial-gradient(circle at 20% 15%, ${ACCENT_LIGHT} 0%, ${PAPER} 45%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:400}}>
        ...
        <Card style={{boxShadow:SHADOW.lg,padding:26}}>
          ...
          {success&&<div style={{background:ACCENT_LIGHT,borderRadius:12,padding:"11px 15px",fontSize:14.5,color:ACCENT,marginBottom:10,fontWeight:600}}>{success}</div>}
          <div style={{display:"flex",flexDirection:"column",gap:13}}>
            <input type="email" placeholder="इमेल" value={email} onChange={(e)=>setEmail(e.target.value)} style={{border:`1.5px solid ${BORDER}`,borderRadius:12,padding:"13px 15px",fontSize:16,fontFamily:"Inter,sans-serif",outline:"none"}}/>
            <input type="password" placeholder="पासवर्ड" value={password} onChange={(e)=>setPassword(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&handle()} style={{border:`1.5px solid ${BORDER}`,borderRadius:12,padding:"13px 15px",fontSize:16,fontFamily:"Inter,sans-serif",outline:"none"}}/>
          ...
          <div style={{textAlign:"center",marginTop:18,fontSize:14.5,color:INK_SOFT}}>
  ...
  return(
    <div style={{padding:"10px 16px",background:"#fff",borderBottom:"1px solid #ECE6D8"}}>
      <div style={{display:"flex",gap:8,overflowX:"auto",alignItems:"center"}}>
        {sections.map((s)=>(
          <button key={s.id} onClick={()=>onChange(s)} style={{padding:"6px 14px",borderRadius:999,border:"none",fontWeight:700,fontSize:14.5,whiteSpace:"nowrap",cursor:"pointer",background:current?.id===s.id?ACCENT:"#F4EFE3",color:current?.id===s.id?"#fff":"#7A6F3E"}}>{s.name}</button>
        ))}
        {adding?(
          <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
            <input autoFocus value={name} onChange={(e)=>setName(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&save()} placeholder="जस्तै: ५ क" style={{border:"1px solid #ECE6D8",borderRadius:8,padding:"6px 10px",fontSize:14.5,width:100,fontFamily:"Inter,sans-serif"}}/>
            <button onClick={save} disabled={loading} style={{background:ACCENT,color:"#fff",border:"none",borderRadius:8,padding:"6px 12px",fontWeight:700,fontSize:14.5,cursor:"pointer"}}>{loading?"...":"थप"}</button>
            <button onClick={()=>setAdding(false)} style={{background:"none",border:"none",cursor:"pointer",color:"#8A8275"}}><X size={16}/></button>
          </div>
        ):(
          <button onClick={()=>setAdding(true)} style={{display:"flex",alignItems:"center",gap:4,padding:"6px 10px",borderRadius:999,border:"1px dashed #C7BFAE",background:"none",color:"#8A8275",fontSize:13.5,fontWeight:600,cursor:"pointer",flexShrink:0}}><Plus size={13}/>थप</button>
        )}
      </div>
    </div>
  );
    ...
    <div style={{position:"fixed",inset:0,background:PAPER,zIndex:50,display:"flex",flexDirection:"column"}}>
      <div style={{background:ACCENT,color:"#fff",padding:"14px 16px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:12.5,opacity:0.75}}>{lesson.chapters?.title||lesson.chapter_title||""}</div>
      ...
      {(objectives.length>0||vocabulary.length>0)&&(
        <div style={{padding:"12px 16px 8px",background:"#fff",borderBottom:"1px solid #ECE6D8"}}>
          <div style={{fontSize:13.5,color:"#6B6557",marginBottom:5,fontWeight:600}}>आजको उद्देश्य</div>
          <ul style={{margin:0,paddingLeft:16,fontSize:15,color:INK,lineHeight:1.6}}>{objectives.map((o,i)=><li key={i}>{o}</li>)}</ul>
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>{vocabulary.map((v)=><span key={v} style={{background:"#F4EFE3",color:"#7A6F3E",fontSize:13.5,fontWeight:600,padding:"3px 8px",borderRadius:6}}>{v}</span>)}</div>
        </div>
      )}
      <div style={{display:"flex",overflowX:"auto",background:"#fff",borderBottom:"1px solid #ECE6D8"}}>
        {tabs.map((t)=>{const Icon=t.icon;const active=tab===t.id;return<button key={t.id} onClick={()=>setTab(t.id)} style={{display:"flex",alignItems:"center",gap:5,padding:"11px 12px",border:"none",background:"none",borderBottom:active?`3px solid ${ACCENT}`:"3px solid transparent",color:active?ACCENT:"#8A8275",fontWeight:600,fontSize:14.5,cursor:"pointer",whiteSpace:"nowrap"}}><Icon size={15}/>{t.label}</button>;})}
      </div>
      <div style={{flex:1,overflowY:"auto",padding:16,maxWidth:720,margin:"0 auto",width:"100%"}}>
        {tab==="sequence"&&(<div><SectionLabel>पढाउने क्रम</SectionLabel>{sequence.length===0?<div style={{color:"#8A8275"}}>पढाउने क्रम थपिएको छैन।</div>:(<ol style={{margin:0,paddingLeft:0,listStyle:"none"}}>{sequence.map((s,i)=>(<li key={i} style={{display:"flex",gap:12,padding:"12px 0",borderBottom:i<sequence.length-1?"1px solid #ECE6D8":"none"}}><div style={{width:26,height:26,borderRadius:"50%",background:"#E4EFE6",color:ACCENT,fontWeight:700,fontSize:14.5,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{i+1}</div><div style={{fontSize:16,color:INK,lineHeight:1.5,paddingTop:2}}>{s}</div></li>))}</ol>)}{lesson.notes&&<div style={{marginTop:14,background:"#FBEBD3",borderRadius:10,padding:12}}><div style={{fontSize:13.5,fontWeight:700,color:"#9A5B12",marginBottom:3}}>नोट</div><div style={{fontSize:15,color:"#5E4622"}}>{lesson.notes}</div></div>}</div>)}
        {tab==="questions"&&<div><SectionLabel>कक्षामा सोध्नुहोस्</SectionLabel><div style={{display:"flex",flexDirection:"column",gap:8}}>{keyQuestions.length===0?<div style={{color:"#8A8275"}}>प्रश्नहरू थपिएका छैनन्।</div>:keyQuestions.map((q,i)=><Card key={i}><div style={{fontSize:16,color:INK}}>{q}</div></Card>)}</div></div>}
        {tab==="activities"&&<div><SectionLabel>क्रियाकलापहरू</SectionLabel><div style={{display:"flex",flexDirection:"column",gap:8}}>{activities.length===0?<div style={{color:"#8A8275"}}>क्रियाकलापहरू थपिएका छैनन्।</div>:activities.map((a,i)=><Card key={i}><div style={{fontSize:16,color:INK}}>{a}</div></Card>)}</div></div>}
        {tab==="homework"&&<div><SectionLabel>दिने गृहकार्य</SectionLabel><Card><div style={{fontSize:16,color:INK,lineHeight:1.6}}>{lesson.homework||"गृहकार्य थपिएको छैन।"}</div></Card></div>}
        {tab==="rubric"&&<div><SectionLabel>मूल्याङ्कन मापदण्ड</SectionLabel>{rubric.length===0?<div style={{color:"#8A8275"}}>मूल्याङ्कन मापदण्ड थपिएको छैन।</div>:<div style={{display:"flex",flexDirection:"column",gap:8}}>{rubric.map((r,i)=><Card key={i}><div style={{fontWeight:700,color:ACCENT,fontSize:15,marginBottom:3}}>{r.level}</div><div style={{fontSize:15,color:INK}}>{r.desc}</div></Card>)}</div>}</div>}
      </div>
    </div>
  );
}
    ...
    <Card onClick={onClick} style={{padding:18, background: accent?`linear-gradient(135deg, ${color}12, ${color}05)`:SURFACE}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        ...
        <div style={{minWidth:0}}>
          ...
          <div style={{fontSize:14,color:INK_SOFT,fontWeight:600,marginTop:2}}>{label}</div>
        </div>
      </div>
    </Card>
  );
    ...
    <div style={{padding:"18px 18px 130px",maxWidth:1040,margin:"0 auto"}}>
      ...
      {today?(
        <div style={{background:`linear-gradient(135deg,${TEAL} 0%, ${ACCENT} 60%, ${ACCENT_DARK} 100%)`,borderRadius:20,padding:26,color:"#fff",marginBottom:22,boxShadow:SHADOW.accent,position:"relative",overflow:"hidden"}}>
          ...
          <Button variant="marigold" size="lg" icon={Sparkles} onClick={()=>onOpenLesson(today)} style={{marginTop:14}}>आजको पाठ सुरु गर्नुहोस्</Button>
        </div>
      ):(
        <Card style={{marginBottom:22,textAlign:"center",padding:34}}>
          <div style={{color:INK_SOFT,fontSize:16,marginBottom:14}}>{section?"पाठ योजनामा पाठ थप्नुहोस्।":"माथिबाट सेक्सन छान्नुहोस्।"}</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16}}>
        <Card>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><ClipboardList size={17} color={ACCENT}/><div style={{fontWeight:700,color:INK,fontSize:16}}>तयारी चाहिने</div></div>
          {pending.length===0?<div style={{fontSize:14.5,color:INK_SOFT}}>सबै तयार! ✓</div>:pending.map((l)=>(
            <div key={l.id} onClick={onGoPlanner} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 2px",borderTop:`1px solid ${BORDER}`,cursor:"pointer"}}>
              <div style={{fontSize:15,color:INK,fontWeight:600}}>{l.title}</div><StatusPill status={l.status}/>
            </div>
          ))}
        </Card>
        <Card>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><CheckCircle2 size={17} color={ACCENT}/><div style={{fontWeight:700,color:INK,fontSize:16}}>जाँच्नुपर्ने गृहकार्य</div></div>
          {hwPending.length===0?<div style={{fontSize:14.5,color:INK_SOFT}}>सबै जाँच भयो! ✓</div>:hwPending.map((h)=>(
            <div key={h.id} onClick={onGoHomework} style={{display:"flex",justifyContent:"space-between",padding:"10px 2px",borderTop:`1px solid ${BORDER}`,cursor:"pointer"}}>
              <div style={{fontSize:15,color:INK}}>{h.title}</div><div style={{fontSize:14,color:INK_SOFT,fontWeight:700}}>{h.checked_count}/{h.total_students}</div>
            </div>
          ))}
        </Card>
      </div>
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:920,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <button onClick={()=>setShowForm(true)} style={{display:"flex",alignItems:"center",gap:5,background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"8px 14px",fontSize:14.5,fontWeight:700,cursor:"pointer"}}><Plus size={14}/>नयाँ पाठ</button>
      </div>
      {showForm&&(
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontWeight:700,fontSize:16}}>नयाँ पाठ</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:9}}>
            {[["title","पाठको नाम *"]].map(([f,p])=>(
              <input key={f} placeholder={p} value={form[f]} onChange={(e)=>setForm({...form,[f]:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            ))}
            ...
            {[["objectives","उद्देश्यहरू (प्रत्येक नयाँ लाइनमा)"],["vocabulary","शब्दावली (कमाले छुट्याउनुहोस्)"],["sequence","पढाउने क्रम (प्रत्येक नयाँ लाइनमा)"],["key_questions","मुख्य प्रश्नहरू (प्रत्येक नयाँ लाइनमा)"],["activities","क्रियाकलापहरू (प्रत्येक नयाँ लाइनमा)"]].map(([f,p])=>(
              <textarea key={f} placeholder={p} value={form[f]} onChange={(e)=>setForm({...form,[f]:e.target.value})} rows={3} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            ))}
          ...
          {lessons.map((l)=>(
            <Card key={l.id} onClick={()=>onOpenLesson(l)}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                <div>
                  <div style={{fontSize:13.5,color:"#8A8275",fontWeight:600,marginBottom:2}}>{l.chapters?.title||l.chapter_title||""}</div>
                  <div style={{fontSize:16.5,fontWeight:700,color:INK}}>{l.title}</div>
                </div>
  ...
  return(
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))",gap:7}}>
      {CATEGORY_ORDER.map((key)=>{
        ...
        return(
          <button key={key} type="button" onClick={()=>onChange(key)} className="ss-chip" style={{display:"flex",flexDirection:"column",alignItems:"center",gap:5,padding:"10px 6px",borderRadius:12,border:`2px solid ${active?meta.color:BORDER}`,background:active?meta.color+"14":"#fff",color:active?meta.color:INK_SOFT,fontWeight:700,fontSize:13.5,cursor:"pointer"}}>
...
function Materials({ chapters, onAddChapter, onChaptersChanged }) {
  ...
  return(
    <div style={{padding:"18px 18px 130px",maxWidth:1040,margin:"0 auto"}}>
      ...
      {error&&<ErrorMsg msg={error}/>}
      {untaggedCount>0&&(
        <div style={{background:WARN_BG,borderRadius:12,padding:"11px 16px",fontSize:14.5,color:WARN,margin:"12px 0",display:"flex",alignItems:"center",gap:8,fontWeight:600}}>
      ...
      <Card style={{marginBottom:16,marginTop:12}}>
        <div style={{fontSize:15,fontWeight:700,color:INK,marginBottom:10}}>नयाँ फाइल थप्नुहोस्</div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <div>
            <div style={{fontSize:13,fontWeight:700,color:INK_SOFT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.03em"}}>यो फाइल कस्तो प्रकारको हो?</div>
          ...
          <label className="ss-btn" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,background:uploadChapter.trim()?`linear-gradient(180deg, ${ACCENT} 0%, ${ACCENT_DARK} 100%)`:"#D8D2C0",color:"#fff",border:"none",borderRadius:12,padding:"13px",fontSize:15.5,fontWeight:700,cursor:uploadChapter.trim()?"pointer":"not-allowed",boxShadow:uploadChapter.trim()?SHADOW.accent:"none"}}>
          ...
          <div style={{fontSize:13.5,color:INK_SOFT}}>PDF/तस्बिर सिधै AI लाई देखाइन्छ। Word/PowerPoint/Excel बाट टेक्स्ट स्वतः निकालिन्छ।</div>
        </div>
      </Card>

      <div style={{display:"flex",gap:7,overflowX:"auto",marginBottom:12,paddingBottom:2}}>
        <button onClick={()=>setCategoryFilter("all")} className="ss-chip" style={{padding:"8px 14px",borderRadius:999,background:categoryFilter==="all"?ACCENT:"#fff",color:categoryFilter==="all"?"#fff":INK,fontWeight:700,fontSize:14.5,whiteSpace:"nowrap",cursor:"pointer",border:`1.5px solid ${categoryFilter==="all"?ACCENT:BORDER}`,boxShadow:categoryFilter==="all"?SHADOW.sm:"none"}}>सबै ({materials.length})</button>
        {CATEGORY_ORDER.map((key)=>{
          const meta=CATEGORY_META[key];const Icon=meta.icon;const active=categoryFilter===key;
          return(
            <button key={key} onClick={()=>setCategoryFilter(key)} className="ss-chip" style={{display:"flex",alignItems:"center",gap:5,padding:"8px 14px",borderRadius:999,background:active?meta.color:"#fff",color:active?"#fff":INK,fontWeight:700,fontSize:14.5,whiteSpace:"nowrap",cursor:"pointer",border:`1.5px solid ${active?meta.color:BORDER}`,boxShadow:active?SHADOW.sm:"none"}}><Icon size={13}/>{meta.label} ({categoryCounts[key]||0})</button>
          );
        })}
      </div>

      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:180,display:"flex",alignItems:"center",gap:8,background:"#fff",border:`1px solid ${BORDER}`,borderRadius:12,padding:"11px 14px"}}>
          <Search size={16} color="#A39B8B"/>
          <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="फाइल खोज्नुहोस्..." style={{border:"none",outline:"none",fontSize:15.5,flex:1,background:"transparent",fontFamily:"Inter,sans-serif"}}/>
        </div>
        <select value={sortBy} onChange={(e)=>setSortBy(e.target.value)} style={{border:`1px solid ${BORDER}`,borderRadius:12,padding:"11px 14px",fontSize:14.5,fontFamily:"Inter,sans-serif",background:"#fff",color:INK,fontWeight:600}}>
      ...
      {loading?<Spinner/>:filtered.length===0?(
        <div style={{textAlign:"center",color:INK_SOFT,padding:50,fontSize:15.5}}>{query?`"${query}" फेला परेन।`:"यो श्रेणीमा फाइल थपिएको छैन।"}</div>
      ):(
            ...
            return(
              <Card key={f.id} onClick={()=>openPreview(f)} style={{padding:14,paddingTop:46,position:"relative"}}>
                <div style={{position:"absolute",top:6,right:6,display:"flex",gap:2,zIndex:2}}>
                  <button onClick={(e)=>openTagEditor(f,e)} style={{background:"#fff",border:`1px solid ${BORDER}`,borderRadius:10,width:40,height:40,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:f.chapters?.title?ACCENT:"#C7A34A",boxShadow:SHADOW.sm}} title="अध्याय/प्रकार तोक्नुहोस्"><Tag size={18}/></button>
                  <button onClick={(e)=>deleteMat(f,e)} style={{background:"#fff",border:`1px solid ${BORDER}`,borderRadius:10,width:40,height:40,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",color:"#C7BFAE",boxShadow:SHADOW.sm}}><Trash2 size={18}/></button>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
                  ...
                  <div style={{fontSize:12,background:catMeta.color+"18",color:catMeta.color,padding:"3px 8px",borderRadius:6,fontWeight:700,display:"flex",alignItems:"center",gap:4}}><CatIcon size={11}/>{catMeta.label}</div>
                </div>
                <div style={{fontSize:14.5,fontWeight:700,color:INK,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginBottom:4}}>{f.name}</div>
                <div style={{fontSize:12.5,color:INK_SOFT,marginBottom:6,fontWeight:600}}>{f.file_type?.toUpperCase()}</div>
                {f.chapters?.title?(
                  <span style={{fontSize:12,background:ACCENT_LIGHT,color:ACCENT,padding:"3px 8px",borderRadius:6,fontWeight:700,display:"inline-block"}}>{f.chapters.title}</span>
                ):(
                  <span style={{fontSize:12,background:WARN_BG,color:WARN,padding:"3px 8px",borderRadius:6,fontWeight:700,display:"inline-block"}}>अध्याय छैन</span>
                )}
                {needsExtraction&&f.extraction_status==="done"&&<div style={{fontSize:12,color:ACCENT,marginTop:5,fontWeight:700}}>✓ AI तयार</div>}
                {needsExtraction&&f.extraction_status==="failed"&&<div style={{fontSize:12,color:DANGER,marginTop:5,fontWeight:700}}>⚠ टेक्स्ट निकाल्न सकिएन</div>}
              </Card>
            );
          })}
        </div>
      ...
      {preview&&(
        <div style={{position:"fixed",inset:0,background:"rgba(20,18,14,0.6)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setPreview(null)}>
          <div onClick={(e)=>e.stopPropagation()} style={{background:"#fff",borderRadius:18,padding:20,maxWidth:640,width:"100%",maxHeight:"88vh",display:"flex",flexDirection:"column",boxShadow:SHADOW.lg}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{fontSize:17,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",paddingRight:10}}>{preview.name}</div>
              <button onClick={()=>setPreview(null)} style={{background:"none",border:"none",cursor:"pointer",color:"#8A8275",flexShrink:0}}><X size={20}/></button>
            </div>
            {!previewUrl?(
              ...
              <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column",gap:12}}>
                ...
                {(preview.file_type==="doc"||preview.file_type==="pptx"||preview.file_type==="sheet")&&(
                  <div style={{fontSize:13,color:"#A39B8B",textAlign:"center"}}>माथिको प्रिभ्यू नदेखिए, केही सेकेन्ड पर्खनुहोस् वा माथिको बटनबाट खोल्नुहोस्।</div>
                )}
              </div>
            )}
          </div>
      ...
      {tagging&&(
        <div style={{position:"fixed",inset:0,background:"rgba(20,18,14,0.6)",zIndex:60,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={()=>setTagging(null)}>
          <div onClick={(e)=>e.stopPropagation()} style={{background:"#fff",borderRadius:18,padding:24,maxWidth:420,width:"100%",boxShadow:SHADOW.lg}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}>
              <div style={{fontSize:17,fontWeight:700}}>अध्याय र प्रकार तोक्नुहोस्</div>
              <button onClick={()=>setTagging(null)} style={{background:"none",border:"none",cursor:"pointer",color:"#8A8275"}}><X size={20}/></button>
            </div>
            <div style={{fontSize:14.5,color:INK_SOFT,marginBottom:14}}>{tagging.name}</div>
            <div style={{fontSize:13,fontWeight:700,color:INK_SOFT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.03em"}}>प्रकार</div>
            <CategoryPicker value={tagCategory} onChange={setTagCategory}/>
            <div style={{height:14}}/>
            <div style={{fontSize:13,fontWeight:700,color:INK_SOFT,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.03em"}}>अध्याय</div>
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:920,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <button onClick={()=>setShowForm(true)} style={{display:"flex",alignItems:"center",gap:5,background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"8px 14px",fontSize:14.5,fontWeight:700,cursor:"pointer"}}><Plus size={14}/>नयाँ</button>
      </div>
      {showForm&&(
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",flexDirection:"column",gap:9}}>
            <input placeholder="गृहकार्यको शीर्षक" value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <input type="number" placeholder="कुल विद्यार्थी" value={form.total_students} onChange={(e)=>setForm({...form,total_students:parseInt(e.target.value)||0})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <textarea placeholder="टिप्पणी" value={form.remark} onChange={(e)=>setForm({...form,remark:e.target.value})} rows={2} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <div style={{display:"flex",gap:8}}>
            ...
            return(
              <Card key={h.id}>
                <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                  <div style={{fontSize:16,fontWeight:700,color:INK}}>{h.title}</div>
                  <div style={{display:"flex",gap:8,alignItems:"center"}}>
                    <span style={{fontSize:13.5,fontWeight:700,color:done?ACCENT:"#9A5B12",background:done?"#E4EFE6":"#FBEBD3",padding:"3px 8px",borderRadius:999}}>{h.checked_count}/{h.total_students}</span>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <button onClick={()=>bump(h,-1)} style={{width:34,height:34,borderRadius:10,border:"1px solid #ECE6D8",background:"#fff",fontSize:18.5,fontWeight:700,cursor:"pointer"}}>−</button>
                  <div style={{fontSize:14.5,color:"#6B6557",fontWeight:600,flex:1}}>जाँच गर्नुहोस्</div>
                ...
                {h.remark&&<div style={{marginTop:10,background:"#FAF7EE",borderRadius:8,padding:"8px 10px",fontSize:14.5,color:"#6B6557"}}>{h.remark}</div>}
              </Card>
            );
          })}
        </div>
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:720,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        {!showForm&&<button onClick={()=>setShowForm(true)} style={{display:"flex",alignItems:"center",gap:5,background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"8px 14px",fontSize:14.5,fontWeight:700,cursor:"pointer"}}><Plus size={14}/>थप</button>}
      </div>
      {showForm&&(
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <input placeholder="आजको पाठ" value={form.lesson_title} onChange={(e)=>setForm({...form,lesson_title:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <textarea placeholder="के पढाइयो?" value={form.taught} onChange={(e)=>setForm({...form,taught:e.target.value})} rows={2} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <textarea placeholder="के गाह्रो भयो?" value={form.difficulty} onChange={(e)=>setForm({...form,difficulty:e.target.value})} rows={2} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <textarea placeholder="अर्को पटककालागि सुझाव" value={form.idea} onChange={(e)=>setForm({...form,idea:e.target.value})} rows={2} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <div style={{display:"flex",gap:8}}>
              {Object.entries(MOOD_META).map(([key,m])=>{const Icon=m.icon;return<button key={key} onClick={()=>setForm({...form,mood:key})} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:5,padding:"8px",borderRadius:10,border:form.mood===key?`2px solid ${m.color}`:"1px solid #ECE6D8",background:form.mood===key?m.color+"15":"#fff",color:m.color,fontSize:13.5,fontWeight:700,cursor:"pointer"}}><Icon size={13}/>{m.label}</button>;})}
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowForm(false)} style={{flex:1,padding:"10px",borderRadius:10,border:"1px solid #ECE6D8",background:"#fff",fontWeight:600,cursor:"pointer"}}>रद्द</button>
              <button onClick={save} disabled={saving} style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:ACCENT,color:"#fff",fontWeight:700,cursor:"pointer"}}>{saving?"...":"सुरक्षित"}</button>
      ...
      {loading?<Spinner/>:entries.length===0?<div style={{textAlign:"center",color:"#8A8275",padding:40}}>कुनै प्रविष्टि छैन।</div>:(
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {entries.map((e)=>{const mood=MOOD_META[e.mood]||MOOD_META.okay;const MIcon=mood.icon;return(
            <Card key={e.id}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                <div style={{fontSize:15.5,fontWeight:700,color:INK}}>{e.lessons?.title||e.entry_date}</div>
                <div style={{display:"flex",alignItems:"center",gap:4,background:mood.color+"15",color:mood.color,padding:"3px 9px",borderRadius:999,fontSize:13,fontWeight:700}}><MIcon size={12}/>{mood.label}</div>
              </div>
              {e.taught&&<div style={{fontSize:14.5,color:INK,marginBottom:5}}><strong>के पढाइयो:</strong> {e.taught}</div>}
              {e.difficulty&&<div style={{fontSize:14.5,color:"#6B6557",marginBottom:5}}><strong>गाह्रो:</strong> {e.difficulty}</div>}
              {e.idea&&<div style={{background:"#F4EFE3",borderRadius:8,padding:"7px 10px",fontSize:14.5,color:"#7A6F3E"}}>💡 {e.idea}</div>}
            </Card>
          );})}
        </div>
      )}
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 170px)",maxWidth:720,margin:"0 auto",width:"100%"}}>
      <div style={{padding:"14px 16px 8px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,fontSize:13.5,color:"#8A8275",marginTop:3,flexWrap:"wrap"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"6px 16px"}}>
        {messages.map((m,i)=>(
          <div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",marginBottom:10}}>
            <div style={{maxWidth:"88%",background:m.role==="user"?ACCENT:"#fff",color:m.role==="user"?"#fff":INK,border:m.role==="ai"?"1px solid #ECE6D8":"none",borderRadius:14,padding:"11px 14px",fontSize:15.5,lineHeight:1.6,whiteSpace:"pre-wrap"}}>{m.text}</div>
          </div>
        ))}
        {loading&&<div style={{display:"flex",marginBottom:10}}><div style={{background:"#fff",border:"1px solid #ECE6D8",borderRadius:14,padding:"11px 14px",color:"#8A8275",fontSize:15}}>सोच्दै छु...</div></div>}
        <div ref={bottomRef}/>
      </div>
      <div style={{padding:"8px 16px",display:"flex",gap:7,overflowX:"auto"}}>
        {QUICK.map((q)=><button key={q} onClick={()=>send(q)} style={{flexShrink:0,background:"#F4EFE3",color:"#7A6F3E",border:"none",borderRadius:999,padding:"7px 12px",fontSize:13.5,fontWeight:600,whiteSpace:"nowrap",cursor:"pointer"}}>{q}</button>)}
      </div>
      <div style={{display:"flex",gap:8,padding:"8px 16px 16px"}}>
        <input value={input} onChange={(e)=>setInput(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&send(input)} placeholder="आफ्नो प्रश्न लेख्नुहोस्..." style={{flex:1,border:"1px solid #ECE6D8",borderRadius:999,padding:"12px 16px",fontSize:15,outline:"none",fontFamily:"Inter,sans-serif"}}/>
  ...
  return(
    <div style={{padding:"16px 16px 140px",maxWidth:920,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <button onClick={()=>setShowForm(!showForm)} style={{display:"flex",alignItems:"center",gap:5,background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"8px 14px",fontSize:14.5,fontWeight:700,cursor:"pointer"}}><Plus size={14}/>नयाँ</button>
      </div>
      {showForm&&(
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontWeight:700,fontSize:15}}>प्रश्न थप्नुहोस्</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <textarea placeholder="प्रश्न (म्यानुअल)" value={form.text} onChange={(e)=>setForm({...form,text:e.target.value})} rows={3} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <div style={{display:"flex",gap:8}}>
              <select value={form.type} onChange={(e)=>setForm({...form,type:e.target.value})} style={{flex:1,border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}>{TYPES.map((t)=><option key={t}>{t}</option>)}</select>
              <select value={form.difficulty} onChange={(e)=>setForm({...form,difficulty:e.target.value})} style={{flex:1,border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}>{"सजिलो,मध्यम,कठिन".split(",").map((d)=><option key={d}>{d}</option>)}</select>
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowForm(false)} style={{flex:1,padding:"10px",borderRadius:10,border:"1px solid #ECE6D8",background:"#fff",fontWeight:600,cursor:"pointer"}}>रद्द</button>
              <button onClick={save} disabled={saving} style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:ACCENT,color:"#fff",fontWeight:700,cursor:"pointer"}}>{saving?"...":"सुरक्षित"}</button>
      ...
      <div style={{display:"flex",gap:7,overflowX:"auto",marginBottom:14}}>
        {DIFFS.map((d)=><button key={d} onClick={()=>setDiffFilter(d)} style={{padding:"6px 12px",borderRadius:999,background:diffFilter===d?ACCENT:"#fff",color:diffFilter===d?"#fff":INK,fontWeight:600,fontSize:14,whiteSpace:"nowrap",cursor:"pointer",border:"1px solid "+(diffFilter===d?ACCENT:"#ECE6D8")}}>{d}</button>)}
      </div>
            ...
            <Card key={q.id} onClick={()=>toggle(q.id)} style={{display:"flex",gap:10}}>
              ...
              <div style={{flex:1}}>
                <div style={{display:"flex",gap:5,flexWrap:"wrap",marginBottom:5}}>
                  <span style={{fontSize:12.5,background:"#E4EFE6",color:ACCENT,padding:"2px 7px",borderRadius:5,fontWeight:700}}>{q.type}</span>
                  <span style={{fontSize:12.5,background:"#F4EFE3",color:"#7A6F3E",padding:"2px 7px",borderRadius:5,fontWeight:600}}>{q.difficulty}</span>
                  {q.chapter_title&&<span style={{fontSize:12.5,color:"#A39B8B",fontWeight:600}}>{q.chapter_title}</span>}
                </div>
                <div style={{fontSize:15.5,color:INK,lineHeight:1.5}}>{q.text}</div>
                {q.options?.length>0&&<div style={{marginTop:6}}>{q.options.map((o,i)=><div key={i} style={{fontSize:14.5,color:i===q.correct_option?ACCENT:"#6B6557",fontWeight:i===q.correct_option?700:400}}>{i+1}) {o}</div>)}</div>}
              </div>
      ...
      {selected.length>0&&(
        <div style={{position:"fixed",bottom:64,left:0,right:0,display:"flex",justifyContent:"center",padding:"0 16px",zIndex:20}}>
          <button onClick={()=>setShowSet(true)} style={{background:ACCENT,color:"#fff",border:"none",borderRadius:999,padding:"12px 20px",fontWeight:700,fontSize:15,display:"flex",alignItems:"center",gap:8,cursor:"pointer",boxShadow:"0 8px 20px rgba(31,77,61,0.25)"}}><Shuffle size={16}/>{selected.length} प्रश्न — सेट बनाउनुहोस्</button>
        </div>
      )}
      {showSet&&(
        <div style={{position:"fixed",inset:0,background:"rgba(20,18,14,0.55)",zIndex:60,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setShowSet(false)}>
          <div onClick={(e)=>e.stopPropagation()} style={{background:"#fff",borderRadius:"18px 18px 0 0",padding:22,maxWidth:600,width:"100%",maxHeight:"80vh",overflowY:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
              <div style={{fontSize:17.5,fontWeight:700}}>प्रश्न सेट ({selected.length})</div>
            ...
            <ol style={{paddingLeft:18,margin:0,display:"flex",flexDirection:"column",gap:10}}>
              {selectedQs.map((q)=><li key={q.id} style={{fontSize:15,color:INK,lineHeight:1.5}}>{q.text}</li>)}
            </ol>
            <div style={{display:"flex",gap:8,marginTop:18}}>
              <button onClick={async()=>{await db.upsertQuestionSet({title:`सेट — ${new Date().toLocaleDateString("ne-NP")}`,question_ids:selected});setSelected([]);setShowSet(false);}} style={{flex:1,background:ACCENT,color:"#fff",border:"none",borderRadius:12,padding:"12px",fontWeight:700,fontSize:15,cursor:"pointer"}}>💾 सुरक्षित</button>
              <button onClick={()=>window.print()} style={{flex:1,background:MARIGOLD,color:"#2A1E07",border:"none",borderRadius:12,padding:"12px",fontWeight:700,fontSize:15,display:"flex",alignItems:"center",justifyContent:"center",gap:6,cursor:"pointer"}}><Printer size={16}/>प्रिन्ट</button>
            </div>
          </div>
        </div>
      )}
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:920,margin:"0 auto"}}>
      ...
      {showForm&&(
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontWeight:700,fontSize:15}}>नयाँ मूल्याङ्कन</div>
            <AIButton label={generating?"बनाउँदै...":"AI बाट rubric"} onClick={autoGenerate} loading={generating}/>
          </div>
          {error&&<ErrorMsg msg={error}/>}
          <MaterialsHint count={matchedCount} chapterTitle={form.chapter_title}/>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <input placeholder="शीर्षक *" value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            ...
            <textarea placeholder={"मापदण्ड:\nउत्कृष्ट: ...\nराम्रो: ...\nसहयोग आवश्यक: ..."} value={form.rubric_text} onChange={(e)=>setForm({...form,rubric_text:e.target.value})} rows={4} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <input type="date" value={form.due_date} onChange={(e)=>setForm({...form,due_date:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowForm(false)} style={{flex:1,padding:"10px",borderRadius:10,border:"1px solid #ECE6D8",background:"#fff",fontWeight:600,cursor:"pointer"}}>रद्द</button>
              <button onClick={save} disabled={saving} style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:ACCENT,color:"#fff",fontWeight:700,cursor:"pointer"}}>{saving?"...":"सुरक्षित"}</button>
            </div>
          ...
          {assessments.map((a)=>{const typeInfo=TYPES.find((t)=>t.id===a.type)||TYPES[0];const Icon=typeInfo.icon;return(
            <Card key={a.id}>
              <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
                ...
                <div><div style={{fontSize:16,fontWeight:700,color:INK}}>{a.title}</div><div style={{fontSize:13.5,color:"#8A8275"}}>{typeInfo.label}{a.due_date?` · ${a.due_date}`:""}</div></div>
              </div>
              {a.rubric?.length>0&&a.rubric.map((r,i)=><div key={i} style={{background:"#FAF7EE",borderRadius:7,padding:"6px 10px",fontSize:14.5,marginBottom:5}}><strong style={{color:ACCENT}}>{r.level}:</strong> {r.desc}</div>)}
            </Card>
          );})}
        </div>
      )}
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:920,margin:"0 auto"}}>
      ...
      {showForm&&(
        <Card style={{marginBottom:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div style={{fontWeight:700,fontSize:15}}>नयाँ क्रियाकलाप</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <ChapterPicker value={form.chapter_title} onChange={(v)=>setForm({...form,chapter_title:v})} chapters={chapters||[]} onAddChapter={onAddChapter} placeholder="— अध्याय छान्नुहोस् (AI का लागि अनिवार्य) —"/>
            <input placeholder="क्रियाकलापको नाम" value={form.title} onChange={(e)=>setForm({...form,title:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <input placeholder="क्षमता" value={form.competency} onChange={(e)=>setForm({...form,competency:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <input placeholder="समय" value={form.duration} onChange={(e)=>setForm({...form,duration:e.target.value})} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif"}}/>
            <textarea placeholder="विवरण" value={form.description} onChange={(e)=>setForm({...form,description:e.target.value})} rows={3} style={{border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",resize:"vertical"}}/>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
              {TYPES.map((t)=>{const Icon=t.icon;return<button key={t.id} onClick={()=>setForm({...form,type:t.id})} style={{display:"flex",alignItems:"center",gap:6,padding:"9px 10px",borderRadius:10,border:`2px solid ${form.type===t.id?ACCENT:"#ECE6D8"}`,background:form.type===t.id?"#E4EFE6":"#fff",color:form.type===t.id?ACCENT:INK,fontWeight:600,fontSize:14,cursor:"pointer"}}><Icon size={14}/>{t.label}</button>;})}
            </div>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowForm(false)} style={{flex:1,padding:"10px",borderRadius:10,border:"1px solid #ECE6D8",background:"#fff",fontWeight:600,cursor:"pointer"}}>रद्द</button>
              <button onClick={save} disabled={saving} style={{flex:1,padding:"10px",borderRadius:10,border:"none",background:ACCENT,color:"#fff",fontWeight:700,cursor:"pointer"}}>{saving?"...":"सुरक्षित"}</button>
      ...
      <div style={{display:"flex",gap:7,overflowX:"auto",marginBottom:14}}>
        <button onClick={()=>setTypeFilter("सबै")} style={{padding:"6px 12px",borderRadius:999,background:typeFilter==="सबै"?ACCENT:"#fff",color:typeFilter==="सबै"?"#fff":INK,fontWeight:600,fontSize:14,whiteSpace:"nowrap",cursor:"pointer",border:"1px solid "+(typeFilter==="सबै"?ACCENT:"#ECE6D8")}}>सबै</button>
        {TYPES.map((t)=>{const Icon=t.icon;return<button key={t.id} onClick={()=>setTypeFilter(t.id)} style={{display:"flex",alignItems:"center",gap:5,padding:"6px 12px",borderRadius:999,background:typeFilter===t.id?ACCENT:"#fff",color:typeFilter===t.id?"#fff":INK,fontWeight:600,fontSize:14,whiteSpace:"nowrap",cursor:"pointer",border:"1px solid "+(typeFilter===t.id?ACCENT:"#ECE6D8")}}><Icon size={13}/>{t.label}</button>;})}
      </div>
          ...
          {filtered.map((a)=>{const typeInfo=TYPES.find((t)=>t.id===a.type)||TYPES[0];const Icon=typeInfo.icon;return(
            <Card key={a.id}>
              <div style={{display:"flex",gap:12}}>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",justifyContent:"space-between"}}><div style={{fontWeight:700,fontSize:15.5,color:INK}}>{a.title}</div>{a.duration&&<span style={{fontSize:13.5,color:"#8A8275"}}>{a.duration}</span>}</div>
                  {a.description&&<div style={{fontSize:14.5,color:"#6B6557",lineHeight:1.5,marginTop:4}}>{a.description}</div>}
                  <div style={{display:"flex",gap:5,marginTop:6,flexWrap:"wrap"}}>
                    {a.chapter_title&&<span style={{fontSize:12.5,background:"#F4EFE3",color:"#7A6F3E",padding:"2px 7px",borderRadius:5,fontWeight:600}}>{a.chapter_title}</span>}
                    {a.competency&&<span style={{fontSize:12.5,background:"#E4EFE6",color:ACCENT,padding:"2px 7px",borderRadius:5,fontWeight:600}}>{a.competency}</span>}
                  </div>
                </div>
              </div>
            </Card>
...
function ResourceCreator({ lessons }) {
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:920,margin:"0 auto"}}>
      ...
      <div style={{fontSize:14.5,color:"#8A8275",marginBottom:16}}>{lesson?`"${lesson.title}" — AI बाट स्वतः बनाइन्छ।`:"पहिले पाठ योजनामा पाठ थप्नुहोस्।"}</div>
      <MaterialsHint count={matchedCount} chapterTitle={chapterTitle}/>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:20}}>
        {TEMPLATES.map((t)=>{const Icon=t.icon;return<Card key={t.id} onClick={()=>generate(t)} style={{padding:14,border:active?.id===t.id?`2px solid ${ACCENT}`:"1px solid #ECE6D8"}}><div style={{width:36,height:36,borderRadius:8,background:"#E4EFE6",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:8}}><Icon size={18} color={ACCENT}/></div><div style={{fontWeight:700,fontSize:14.5,color:INK}}>{t.title}</div></Card>;})}
      </div>
      {active&&(
        <Card>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div style={{fontWeight:700,fontSize:16}}>{active.title}</div>
            {!generating&&generatedText&&<button onClick={()=>window.print()} style={{display:"flex",alignItems:"center",gap:5,background:MARIGOLD,color:"#2A1E07",border:"none",borderRadius:10,padding:"7px 12px",fontWeight:700,fontSize:14.5,cursor:"pointer"}}><Printer size={14}/>प्रिन्ट</button>}
          </div>
          {generating?<div style={{display:"flex",alignItems:"center",gap:8,color:"#8A8275",fontSize:15,padding:20}}><Spinner small/>AI बनाउँदैछ...</div>:(
            <pre style={{background:"#FAF7EE",borderRadius:10,padding:14,fontSize:14.5,color:INK,lineHeight:1.7,whiteSpace:"pre-wrap",fontFamily:"Inter,sans-serif",maxHeight:400,overflowY:"auto"}}>{generatedText}</pre>
          )}
        </Card>
      )}
    </div>
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:720,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,background:"#fff",border:"1px solid #ECE6D8",borderRadius:14,padding:"12px 14px",marginBottom:14,marginTop:10}}>
        ...
        <input autoFocus value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="खोज्नुहोस्..." style={{border:"none",outline:"none",fontSize:16,flex:1,background:"transparent",fontFamily:"Inter,sans-serif"}}/>
      </div>
      {!query.trim()?<div style={{textAlign:"center",color:"#A39B8B",padding:40,fontSize:15}}>टाइप गर्नुहोस्...</div>:results.length===0?<div style={{textAlign:"center",color:"#8A8275",padding:40}}>"{query}" फेला परेन।</div>:(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <div style={{fontSize:14,color:"#8A8275",marginBottom:4}}>{results.length} परिणाम</div>
          {results.map((r,i)=>{const Icon=r.icon;return<Card key={i} style={{display:"flex",gap:10,alignItems:"center"}}><div style={{width:36,height:36,borderRadius:8,background:r.color+"1A",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><Icon size={17} color={r.color}/></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:12.5,color:r.color,fontWeight:700,marginBottom:2}}>{r.kind}</div><div style={{fontSize:15,color:INK,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.title}</div>{r.sub&&<div style={{fontSize:13.5,color:"#8A8275"}}>{r.sub}</div>}</div></Card>;})}
        </div>
      )}
    </div>
  );
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:500,margin:"0 auto"}}>
      ...
      <Card style={{marginBottom:14,padding:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <button onClick={()=>{if(month===0){setMonth(11);setYear(y=>y-1);}else setMonth(m=>m-1);}} style={{background:"#F4EFE3",border:"none",borderRadius:8,padding:"6px 12px",fontWeight:700,cursor:"pointer",color:ACCENT,fontSize:17}}>‹</button>
          <div style={{textAlign:"center"}}>
            <select value={month} onChange={(e)=>setMonth(Number(e.target.value))} style={{border:"none",fontWeight:700,fontSize:17,color:INK,cursor:"pointer",background:"transparent",fontFamily:"Inter,sans-serif"}}>
              {MONTHS.map((m,i)=><option key={i} value={i}>{m}</option>)}
            </select>
            <select value={year} onChange={(e)=>setYear(Number(e.target.value))} style={{border:"none",fontWeight:700,fontSize:17,color:INK,cursor:"pointer",background:"transparent",fontFamily:"Inter,sans-serif",marginLeft:4}}>
              {Array.from({length:5},(_,i)=>today.getFullYear()-1+i).map((y)=><option key={y} value={y}>{y}</option>)}
            </select>
          </div>
          <button onClick={()=>{if(month===11){setMonth(0);setYear(y=>y+1);}else setMonth(m=>m+1);}} style={{background:"#F4EFE3",border:"none",borderRadius:8,padding:"6px 12px",fontWeight:700,cursor:"pointer",color:ACCENT,fontSize:17}}>›</button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4,marginBottom:6}}>
          {DAYS.map((d)=><div key={d} style={{textAlign:"center",fontSize:13.5,fontWeight:700,color:"#8A8275",padding:"4px 0"}}>{d}</div>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:4}}>
          ...
          {Array.from({length:daysInMonth},(_,i)=>i+1).map((day)=>{
            ...
            return<button key={day} onClick={()=>setSelected(day)} style={{aspectRatio:1,borderRadius:8,border:"none",background:isToday?ACCENT:isSel?"#E4EFE6":"transparent",color:isToday?"#fff":isSel?ACCENT:INK,fontWeight:isToday||isSel?700:400,fontSize:15,cursor:"pointer"}}>{day}</button>;
          })}
        </div>
      </Card>
      <SectionLabel>आजका कार्यहरू</SectionLabel>
      {[...lessons.slice(0,3).map((l)=>({type:"पाठ",title:l.title,color:ACCENT,bg:"#E4EFE6"})),...homework.filter((h)=>h.checked_count<h.total_students).slice(0,2).map((h)=>({type:"गृहकार्य",title:h.title,color:"#9A5B12",bg:"#FBEBD3"}))].length===0?<div style={{color:"#8A8275",fontSize:15}}>कुनै कार्य छैन।</div>:(
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {[...lessons.slice(0,3).map((l)=>({type:"पाठ",title:l.title,color:ACCENT,bg:"#E4EFE6"})),...homework.filter((h)=>h.checked_count<h.total_students).slice(0,2).map((h)=>({type:"गृहकार्य",title:h.title,color:"#9A5B12",bg:"#FBEBD3"}))].map((item,i)=>(
            <Card key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px"}}>
              <span style={{fontSize:12.5,fontWeight:700,color:item.color,background:item.bg,padding:"3px 8px",borderRadius:5,flexShrink:0}}>{item.type}</span>
              <div style={{fontSize:15,color:INK,fontWeight:600}}>{item.title}</div>
            </Card>
          ))}
        </div>
      )}
  ...
  return(
    <div style={{padding:"16px 16px 120px",maxWidth:600,margin:"0 auto"}}>
      ...
      <Card style={{marginBottom:14}}>
        ...
        <div style={{fontSize:14.5,color:"#6B6557",marginBottom:12,lineHeight:1.6}}>एकपटक PDF अपलोड गर्नुहोस् — AI ले सबैतिर यसबाट स्वतः सामग्री बनाउनेछ, साथै सामग्री खण्डमा अध्याय अनुसार ट्याग गरिएका फाइलहरू पनि प्रयोग हुन्छन्।</div>
        {pdfLoaded?(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:8,background:"#E4EFE6",borderRadius:10,padding:"10px 14px"}}>
              ...
              <div style={{fontSize:15,color:ACCENT,fontWeight:700}}>पाठ्यपुस्तक लोड भएको छ ✓</div>
            </div>
            <button onClick={clearTextbookHandler} style={{display:"flex",alignItems:"center",gap:6,background:"#F6E1DC",color:"#A23C2A",border:"none",borderRadius:10,padding:"10px 14px",fontWeight:700,fontSize:14.5,cursor:"pointer"}}><Trash2 size={15}/>पाठ्यपुस्तक हटाउनुहोस्</button>
          </div>
        ):(
          <label style={{display:"flex",alignItems:"center",gap:8,background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"12px 16px",fontWeight:700,fontSize:15,cursor:"pointer"}}>
        ...
        {msg&&<div style={{marginTop:10,fontSize:14.5,color:pdfLoaded?ACCENT:"#A23C2A",fontWeight:600}}>{msg}</div>}
      </Card>
      ...
      <Card style={{marginBottom:14}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
          ...
          <div><div style={{fontWeight:700,color:INK,fontSize:15}}>{session?.user?.email||""}</div><div style={{fontSize:13.5,color:"#8A8275"}}>कक्षा ५ · सामाजिक अध्ययन</div></div>
        </div>
        <button onClick={()=>db.signOut()} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"10px",borderRadius:10,border:"1px solid #F6E1DC",background:"#FFF5F3",color:"#A23C2A",fontWeight:700,fontSize:15,cursor:"pointer"}}><LogOut size={15}/>लगआउट</button>
      </Card>

      <Card style={{marginBottom:14}}>
        <SectionLabel>सेक्सनहरू</SectionLabel>
        <div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:12}}>
          {sections.length===0?<div style={{fontSize:14.5,color:"#8A8275"}}>कुनै सेक्सन छैन।</div>:sections.map((s)=><div key={s.id} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",background:"#FAF7EE",borderRadius:8}}><div style={{width:8,height:8,borderRadius:"50%",background:ACCENT}}/><div style={{fontSize:15,fontWeight:600,color:INK}}>{s.name}</div></div>)}
        </div>
        <div style={{display:"flex",gap:8}}>
          <input value={name} onChange={(e)=>setName(e.target.value)} onKeyDown={(e)=>e.key==="Enter"&&addSection()} placeholder="नयाँ सेक्सन (जस्तै: कक्षा ५ ख)" style={{flex:1,border:"1px solid #ECE6D8",borderRadius:10,padding:"10px 12px",fontSize:15,fontFamily:"Inter,sans-serif",outline:"none"}}/>
          <button onClick={addSection} disabled={saving} style={{background:ACCENT,color:"#fff",border:"none",borderRadius:10,padding:"10px 16px",fontWeight:700,fontSize:15,cursor:"pointer"}}>{saving?"...":"थप"}</button>
        </div>
      </Card>

      <Card>
        <SectionLabel>एपको बारेमा</SectionLabel>
        {[["नाम","शिक्षा साथी"],["संस्करण","3.1"],["AI","Google Gemini (निःशुल्क)"],["डाटाबेस","Supabase (निःशुल्क)"],["होस्टिङ","Vercel (निःशुल्क)"]].map(([l,v])=><div key={l} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #F3EFE3"}}><div style={{fontSize:14.5,color:"#8A8275"}}>{l}</div><div style={{fontSize:14.5,fontWeight:600,color:INK}}>{v}</div></div>)}
      </Card>
    </div>
  );
}
...
export default function App() {
  ...
  return(
    <div style={{fontFamily:"Inter,sans-serif",background:PAPER,minHeight:"100vh",color:INK,fontSize:16}}>
      <style>{`
        *{box-sizing:border-box;}body{margin:0;-webkit-font-smoothing:antialiased;}
        ...
        .ss-card{transition:box-shadow .2s ease, transform .2s ease;}
      ...
      <div className="mobile-bottom-nav" style={{position:"fixed",bottom:0,left:0,right:0,background:SURFACE,borderTop:`1px solid ${BORDER}`,justifyContent:"space-around",padding:"8px 4px",zIndex:10,boxShadow:"0 -4px 16px rgba(33,30,26,0.06)"}}>
        {nav.map((n)=>{const Icon=n.icon;const active=screen===n.id;return<button key={n.id} onClick={()=>setScreen(n.id)} style={{background:"none",border:"none",display:"flex",flexDirection:"column",alignItems:"center",gap:3,color:active?ACCENT:"#A39B8B",fontSize:12,fontWeight:700,cursor:"pointer",padding:"4px 6px",flex:1,transition:"color .15s"}}><Icon size={20}/>{n.label}</button>;})}
        <button onClick={()=>setShowMore(true)} style={{background:"none",border:"none",display:"flex",flexDirection:"column",alignItems:"center",gap:3,color:navMore.some((n)=>n.id===screen)?ACCENT:"#A39B8B",fontSize:12,fontWeight:700,cursor:"pointer",padding:"4px 6px",flex:1}}><Layers size={20}/>थप</button>
      </div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(20,18,14,0.6)",zIndex:60,display:"flex",alignItems:"flex-end",justifyContent:"center"}} onClick={()=>setShowMore(false)}>
          <div onClick={(e)=>e.stopPropagation()} style={{background:SURFACE,borderRadius:"22px 22px 0 0",padding:20,maxWidth:480,width:"100%",paddingBottom:44,boxShadow:SHADOW.lg}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <div style={{fontSize:17.5,fontWeight:800}}>थप विशेषताहरू</div>
              <button onClick={()=>setShowMore(false)} style={{background:"none",border:"none",cursor:"pointer",color:"#8A8275"}}><X size={22}/></button>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
              {navMore.map((n)=>{const Icon=n.icon;const active=screen===n.id;return<button key={n.id} onClick={()=>{setScreen(n.id);setShowMore(false);}} className="ss-btn" style={{display:"flex",flexDirection:"column",alignItems:"center",gap:7,padding:"16px 6px",borderRadius:14,border:`1.5px solid ${active?ACCENT:BORDER}`,background:active?ACCENT_LIGHT:"#fff",color:active?ACCENT:INK,cursor:"pointer",fontSize:14,fontWeight:700,boxShadow:active?SHADOW.sm:"none"}}><Icon size={22}/>{n.label}</button>;})}
            </div>
          </div>
        </div>
      )}
